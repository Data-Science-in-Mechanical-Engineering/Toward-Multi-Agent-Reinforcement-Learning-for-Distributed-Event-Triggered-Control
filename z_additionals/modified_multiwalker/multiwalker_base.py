...
class BipedalWalker(Agent):
    ...
    def get_observation(self):
        ...
        state = [
            #!
            # Position, normalized on the whole scale
            #pos.x / TERRAIN_LENGTH,
    ...
    def observation_space(self):
        # added 2 for position, 14 original obs (joints, etc), 3 for package
        return spaces.Box(
            low=np.float32(-np.inf),
            high=np.float32(np.inf),
            shape=(3+ 14 + 4*(self._n_walkers),), #!
            dtype=np.float32,
        )
...
class MultiWalkerEnv:
    ...
    def setup(self):
        ...
        self.walkers = [
            BipedalWalker(self.world, init_x=sx, init_y=init_y, seed=self.seed_val, n_walkers=self.n_walkers)
        ...
        for i in range(self.n_walkers):
            ...
            #for j in [i - 1, i + 1]:
                ...
                if j==i:
                    ...
                    neighbor_obs.append(xm)
                    neighbor_obs.append(ym)
                # neighbor speed differences
                # if no neighbor (for edge walkers)
                #if j < 0 or j == self.n_walkers or self.walkers[j].hull is None:
                if j==i:
                    neighbor_obs.append(0.0)
            ...
            package_obs.append(self.np_random.normal(self.package.angle, self.angle_noise))
            obs.append(np.array(package_obs + walker_obs + neighbor_obs))
        ...
        for i in range(self.terrain_length):
            ...
            if state == GRASS and not oneshot:
                ...
                if i > TERRAIN_STARTPAD:
                    velocity += np.random.uniform(-1, 1)/SCALE #self.np_random.uniform(-1, 1) / SCALE
                y += velocity
